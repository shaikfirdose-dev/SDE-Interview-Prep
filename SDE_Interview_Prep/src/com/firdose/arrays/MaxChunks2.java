
//Question :https://leetcode.com/problems/max-chunks-to-make-sorted-ii/

package com.firdose.arrays;

public class MaxChunks2 {
	public int maxChunksToSorted(int[] arr) {
		int n = arr.length;
		int count = 0;

		int prefixMax[] = findPrefixMax(arr);
		int suffixMin[] = findSuffixMin(arr);

		for (int i = 0; i < n - 1; i++) {
			if (prefixMax[i] <= suffixMin[i + 1]) {
				count++;
			}
		}
		count++;
		return count;
	}

	private static int[] findPrefixMax(int[] arr) {
		int prefixMax[] = new int[arr.length];
		prefixMax[0] = arr[0];
		for (int i = 1; i < arr.length; i++) {
			prefixMax[i] = Math.max(prefixMax[i - 1], arr[i]);
		}
		return prefixMax;
	}

	private static int[] findSuffixMin(int[] arr) {
		int suffixMin[] = new int[arr.length];
		suffixMin[arr.length - 1] = arr[arr.length - 1];
		for (int i = arr.length - 2; i >= 0; i--) {
			suffixMin[i] = Math.min(suffixMin[i + 1], arr[i]);
		}
		return suffixMin;
	}

}
